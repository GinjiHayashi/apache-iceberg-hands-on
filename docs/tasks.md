...
| 12 | 02_time_travel | スナップショット、過去時点のクエリ、ロールバック | done |
...
| 19 | 09_pyiceberg | テーブル一覧、pandas / DuckDB への読み込み、簡単な追記 | done |
| 20 | 10_v3_upgrade（追加目標・任意） | v2 から v3 へのアップグレードと、各エンジンから読めるかの確認 | done |

## 仕上げ

| # | タスク | 完了条件 | 状態 |
| --- | --- | --- | --- |
| 21 | README の整備 | 前提条件、起動手順、Make コマンドと実際に実行されるコマンド、ハンズオンの一覧が書かれている。README だけを読んで、まっさらな状態から `make smoke` まで到達できる | done |
